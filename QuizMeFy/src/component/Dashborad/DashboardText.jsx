import React from "react";

export default function DashboardText() {
  return (
    <>
      <div className=" rounded-full px-6 py-2 text-teal-500 bg-teal-800/40 font-semibold mb-4 w-fit flex mb-8">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="24"
          height="24"
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          strokeWidth="2"
          strokeLinecap="round"
          strokeLinejoin="round"
          className="lucide lucide-brain-icon lucide-brain"
        >
          <path d="M12 18V5" />
          <path d="M15 13a4.17 4.17 0 0 1-3-4 4.17 4.17 0 0 1-3 4" />
          <path d="M17.598 6.5A3 3 0 1 0 12 5a3 3 0 1 0-5.598 1.5" />
          <path d="M17.997 5.125a4 4 0 0 1 2.526 5.77" />
          <path d="M18 18a4 4 0 0 0 2-7.464" />
          <path d="M19.967 17.483A4 4 0 1 1 12 18a4 4 0 1 1-7.967-.517" />
          <path d="M6 18a4 4 0 0 1-2-7.464" />
          <path d="M6.003 5.125a4 4 0 0 0-2.526 5.77" />
        </svg>
        <p className="ml-3">Challenge Your Mind</p>
      </div>
      <h1 className="bg-linear-to-r from-teal-500 to-sky-500 text-transparent bg-clip-text text-5xl font-bold capitalize w-fit">
        QuizMeFy
      </h1>
      <p className="text-lg mt-4 max-w-xl text-slate-500 text-center">
        Welcome to your Dashboard! Here you can create, manage, and take quizzes
        tailored to your learning needs.
      </p>
    </>
  );
}
